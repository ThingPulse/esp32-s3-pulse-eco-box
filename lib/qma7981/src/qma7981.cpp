...
void QMA7981::write_byte(uint8_t address, uint8_t data)
{
    _twoWire->begin();
    _twoWire->beginTransmission(I2C_ADDRESS); // Initialize the Tx buffer
    _twoWire->write(address);                 // Put slave register address in Tx buffer
    _twoWire->write(data);                    // Put data in Tx buffer
    _twoWire->endTransmission();              // Send the Tx buffer
}

uint8_t QMA7981::read_byte(uint8_t address)
{
    _twoWire->begin();
    _twoWire->beginTransmission(I2C_ADDRESS); // Initialize the Tx buffer
    _twoWire->write(address);                 // Put slave register address in Tx buffer
    _twoWire->endTransmission(false);         // Send the Tx buffer, but send a restart to keep connection alive
    _twoWire->requestFrom(I2C_ADDRESS, 1);    // Read one byte from slave register address
    uint8_t data = _twoWire->read();          // Fill Rx buffer with result
    return data;                         // Return data read from slave register
}

int16_t QMA7981::read_accel_axis(uint8_t address_msb)
{
    _twoWire->begin();
    _twoWire->beginTransmission(I2C_ADDRESS);
    _twoWire->write(address_msb);
    _twoWire->endTransmission(false);
    _twoWire->requestFrom(I2C_ADDRESS, 2);
    int16_t data = (_twoWire->read() & 0b11111100) | (_twoWire->read() << 8); // dump into a 16 bit signed int, so the sign is correct
    data = data / 4;                                                // divide the result by 4 to maintain the sign, since the data is 14 bits
    return data;
}

void QMA7981::set_bit(uint8_t *byte, uint8_t n, bool value)
{
    *byte = (*byte & ~(1UL << n)) | (value << n);
}

bool QMA7981::get_bit(uint8_t byte, uint8_t n)
{
    return (byte >> n) & 1U;
}

void QMA7981::soft_reset()
{
    write_byte(0x36, 0xB6);
    write_byte(0x36, 0x00);
}

void QMA7981::set_interrupt_all_latch(bool latch)
{
...
void QMA7981::reset_motion_detector(bool any_motion, bool significant_motion, bool no_motion)
{
...
void QMA7981::set_any_motion_axis(bool x_enabled, bool y_enabled, bool z_enabled)
{
...
void QMA7981::set_any_motion_number_of_samples(qma7981_any_motion_samples_t samples)
{
...
void QMA7981::set_any_motion_threshold(uint8_t threshold)
{
    write_byte(0x2E, threshold);
}

void QMA7981::set_no_motion_axis(bool x_enabled, bool y_enabled, bool z_enabled)
{
    uint8_t data = read_byte(0x18);
...
void QMA7981::set_no_motion_duration(qma7981_no_motion_duration_t duration)
{
    uint8_t data = read_byte(0x2C);
...
void QMA7981::set_no_motion_threshold(uint8_t threshold)
{
    write_byte(0x2D, threshold);
}

void QMA7981::set_any_or_significant_motion(bool significant_motion)
{
...
void QMA7981::initialize_default(TwoWire *twoWire)
{
    _twoWire = twoWire;
    delay(10);
    soft_reset();
